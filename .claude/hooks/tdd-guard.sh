...
INPUT=$(cat)
# JSON 파싱: jq가 없는 환경(Windows Git Bash 등)도 동작하도록 node 사용.
FILE_PATH=$(printf '%s' "$INPUT" | node -e 'let d="";process.stdin.on("data",c=>d+=c).on("end",()=>{try{const o=JSON.parse(d);process.stdout.write((o.tool_input&&o.tool_input.file_path)||"")}catch(e){process.stdout.write("")}})')
...
# Windows 경로(역슬래시)를 슬래시로 정규화 — dirname/basename/패턴 매칭 호환
FILE_PATH="${FILE_PATH//\\//}"
